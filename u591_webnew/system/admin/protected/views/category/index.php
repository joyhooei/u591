<?php if(Yii::app()->request->isAjaxRequest && Yii::app()->request->isPostRequest) { ?>
	...
	<p><a href="<?=$this->createUrl('category/add'); ?>" title="新增" class="btn"><i class="icon-plus"></i> 新增</a></p>
	<table class="table table-hover table-striped">
	    ...
	    <tbody>
	        ...
	        <tr>
	            <td><?=$v->id; ?></td>
				<td><?=$v->name; ?></td>
				<td><?=$v->is_index ? '<i class="icon-ok"></i>' : ''; ?></td>
				<td><?=$v->status ? '<i class="icon-ok"></i>' : ''; ?></td>
				<td><?=$v->game_id; ?></td>
	            <td style="text-align:center"><a data-trigger="confirm" data-content="你确定要删除吗？删除后无法恢复" href="<?=$this->createUrl('category/del/id/'.$v->id); ?>"><i class="icon-remove"></i> 删除</a></td>
	            <td style="text-align:center"><a href="<?=$this->createUrl('category/update/id/'.$v->id); ?>"  title="修改"><i class="icon-edit"></i> 修改</a></td>
	        </tr>
	       <?php } ?>     
	    </tbody>
	</table>
