...
$get = serialize($_GET);
$ip = getIP_front();
write_log(ROOT_PATH."log","exchange_code_gplog_"," post=$post, get=$get, ip=$ip, ".date("Y-m-d H:i:s")."\r\n");

$sign = $_REQUEST["sign"];
global $key_arr;
$key = $key_arr[8]['gp']['appSecret'];
$data = $_REQUEST;
unset($data['sign']);
ksort($data);
...
$data['sign'] = md5($str);
if($sign != $data['sign']) {
	write_log(ROOT_PATH."log","exchange_code_gplog_error_",$str.",sign error,{$data['sign']}, post=$post,get=$get, ".date("Y-m-d H:i:s")."\r\n");
	exit('FAILURE');
}
...
if($account == ''){
	write_log(ROOT_PATH."log","exchange_code_gplog_error_","accountid is null, post=$post,get=$get, ".date("Y-m-d H:i:s")."\r\n");
	exit('FAILURE');
}
$myconn = SetConn(88);
...
if(false == $result_type = mysqli_query($myconn,$sql)){
	write_log(ROOT_PATH."log","exchange_code_gplog_error_","$sql, ".date("Y-m-d H:i:s")."\r\n");
	exit('FAILURE');
}
$num_rows  = mysqli_num_rows($result_type);
if($num_rows > 0){
	write_log(ROOT_PATH."log","exchange_code_gplog_error_","already used by user, $sql, ".date("Y-m-d H:i:s")."\r\n");
	exit('FAILURE');
}
...
if(false == mysqli_query($conn, $sql)){
	write_log(ROOT_PATH."log","exchange_code_gplog_error_","sql=$sql, ".mysqli_error($conn).date("Y-m-d H:i:s")."\r\n");
    exit('FAILURE');
}
...
write_log(ROOT_PATH."log","exchange_code_gplog_","发送成功，sql=$sql, ".date("Y-m-d H:i:s")."\r\n");
exit('SUCCESS');
