...
if(!empty($code)){
	...
	if (eregi('^[_\.0-9a-z-]+@([0-9a-z][0-9a-z-]+\.)+[a-z]{2,3}$', $username)){
		...
		exit(json_encode(array('status'=>2, 'msg'=>'Ошибка формы телефона или почты.')));
}
...
if(empty($accountConn) || empty($gameId))
	exit(json_encode(array('status'=>2, 'msg'=>'Ошибка ID игры.')));
if(empty($array))
	exit(json_encode(array('status'=>2, 'msg'=>'Ошибка данных.')));
$appKey = $key_arr['appKey'];
$appSecret = $key_arr['appSecret'];
if(!$appKey)
	exit(json_encode(array('status'=>2, 'msg'=>'Ошибка appKey')));
if(!$appSecret)
	exit(json_encode(array('status'=>2, 'msg'=>'Ошибка appSecret')));
ksort($array);
$md5Str = http_build_query($array);
$mySign = md5(urldecode($md5Str).$appKey);
if($mySign != $sign)
	exit(json_encode(array('status'=>2, 'msg'=>'Ошибка sign')));
...
if(isset($code) && !empty($code)){
	...
	if(empty($rs)){
		exit(json_encode(array('status'=>2, 'msg'=>'Капча не существует')));
	}
	if(time()-$rs['addtime'] > 900){
		exit(json_encode(array('status'=>2, 'msg'=>'Капча неэффективна')));
	}
	...
	$result = @mysqli_fetch_assoc($query);
	if(!$result){
		exit(json_encode(array('status'=>2, 'msg'=>'Ошибка аккаунта, введите снова')));
	}
	if($result['password'] != $password){
		exit(json_encode(array('status'=>2, 'msg'=>'Ошибка пароля, введите снова')));
	}
...
exit(json_encode(array('status'=>2, 'msg'=>'Ошибка token')));
