...
write_log(ROOT_PATH."log","kaijia_callback_ios_info_","post=$post,get=$get, ".date("Y-m-d H:i:s")."\r\n");
...
if($mySign != $sign){
    write_log(ROOT_PATH."log","kaijia_callback_ios_error_","sign=$sign, mySign=$mySign, signStr=$signstr, ".date("Y-m-d H:i:s")."\r\n");
...
if($conn == false){
    write_log(ROOT_PATH."log","kaijia_callback_ios_error_","account mysql connect error. ".date("Y-m-d H:i:s")."\r\n");
    exit('ERROR'); //系统错误
}
...
if(!$result_account['NAME']){
    write_log(ROOT_PATH."log","kaijia_callback_ios_error_", "account error! post=$post,get=$get,".date("Y-m-d H:i:s")."\r\n");
    exit('ERROR');  //用户存在
}
...
$conn = SetConn(88);
if($conn == false){
    write_log(ROOT_PATH."log","kaijia_callback_ios_error_","web mysql connect error. ".date("Y-m-d H:i:s")."\r\n");
    exit('ERROR');
}
//判断订单id情况
...
if($result_count['id']){
    write_log(ROOT_PATH."log","kaijia_callback_ios_error_", "order exist!  post=$post,get=$get, ".date("Y-m-d H:i:s")."\r\n");
    exit('SUCCESS'); //订单重复
}
...
$sql=$sql." VALUES (167,$accountId,'$PayName','$serverId','$amount','$orderid','$dwFenBaoID','$Add_Time','1','$gameId','$clienttype',1)";
if (mysqli_query($conn,$sql) == false){
    write_log(ROOT_PATH."log","kaijia_callback_ios_error_", $sql."  ".mysqli_error($conn)."  ".date("Y-m-d H:i:s")."\r\n");
    exit('ERROR');
}
