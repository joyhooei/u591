...
$get = serialize($_GET);
write_log(ROOT_PATH."log","uc_testinfo_log_","post=$post,get=$get, ".date("Y-m-d H:i:s")."\r\n");
$sid = $_REQUEST['sid'];
...
$url = "http://sdk.test4.g.uc.cn/cp/account.verifySession";
...
write_log(ROOT_PATH."log","uc_testresult_log_","result=$result ".date("Y-m-d H:i:s")."\r\n");
...
if(isset($result_arr['state']['code']) && $result_arr['state']['code']==1){
    ...
    if(false == $query = mysqli_query($conn,$sql)){
    	write_log(ROOT_PATH."log","uc_testlogin_error_","$accountConn, sql=$sql, mysql error, ".mysqli_error($conn)." ".date("Y-m-d H:i:s")."\r\n");
    	exit('3 0');
    }
    ...
    if($insert_id){
        write_log(ROOT_PATH."log","new_account_uc_test_log_"," uc new account login, post=$post,get=$get, "."return= 1 $insert_id  ".date("Y-m-d H:i:s")."\r\n");
        exit("1 $insert_id");
    }
}else{
	exit('4 0');;
