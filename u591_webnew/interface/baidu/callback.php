...
function notify_process($config = array()){
	...
	$AppId = $config[$gameId][$type]['appid'];
	$Secretkey = $config[$gameId][$type]['appsecret'];
	$Result["AppID"] =  $AppId;
	$Result["Content"] =  "";
	//参数检测
	...
	if(!$result){
		...
		$sql="insert into web_pay_log (CPID,PayID,data,PayName,ServerID,PayMoney,OrderID,dwFenBaoID,Add_Time,SubStat,game_id,clienttype, rpCode,packageName)";
		$sql=$sql." VALUES (174, $accountId,'$payMoney','$PayName','$serverId','$payMoney','$orderId','$dwFenBaoID','$Add_Time','1','$gameId','$clienttype', '1','$isgoods')";
