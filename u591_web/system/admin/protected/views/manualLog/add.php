...
<body>
	<div class="container-fluid">
			...
			<tr>
				...
				<td><select name="gameid" id="gameId" class="input-medium" required>
                                <?php 
                                	foreach ($game as $k => $v) {
										$selected = '';
                                ...
                                ?>
                            </select> <select name="serverid"
					id="serverId" class="input-medium" required>
			...
			<tr>
				...
				<td>
                    	...
                    	<input type="text" placeholder="账号或者角色名"
					name="username"
			...
			<tr>
				<th style="width: 130px; line-height: 30px; text-align: right"><span
					class="loading"><img src="<?=ASSETS_URL; ?>img/loading.gif"></span>
				</th>
				<td>
					<button type="submit" class="btn btn-primary input-small"
						title="校验">
						<i class="icon-search"></i> 校验
					</button>
				</td>
			</tr>
		</table>
       ...
       <?=CHtml::beginForm('', 'POST', array(
			...
			name="serverid" value="<?=$serverId?>"> <input type="hidden"
		...
		<table class="table table-hover">
			...
			<tr>
				...
				<td><input type="text" placeholder="默认不写，自动生成" name="order_id"
					value="" class="input-large"></td>
			</tr>
			<tr>
				<th style="width: 35%; text-align: right;">充值笔数</th>
				<td><input type="text" placeholder="默认1" name="order_num"
					value="1" class="input-large"></td>
			</tr>
              <?php if(isset($accountInfo['dwFenBaoID'])) { ?>
				...
				<td><select name="payCode" class="input-small">
						<option value="CNY">人民币</option>
						<option value="USD">美元</option>
						<option value="TWD">台币</option>
						<option value="VND">越南盾</option>
				</select> <input type="text" placeholder="金额" name="emoney"
					value="<?=isset($_POST['emoney'])  ? $_POST['emoney'] : '';?>"
					class="input-medium" required=required>&nbsp; <font color="red">国服选择RMB、台服港台根据货币类型选择美元或台币。</font>
				</td>
			</tr>
			<tr>
				<th style="width: 35%; text-align: right;">礼包id</th>
				<td><input type="text" placeholder="0" name="gift_id" value="0"
					class="input-large"></td>
			</tr>
			<tr>
				<th style="width: 35%; text-align: right;">备注</th>
				<td><input type="text" placeholder="备注" name="remark"
					value="<?=isset($_POST['remark'])  ? $_POST['remark'] : '';?>"
					class="input-large" required=required></td>
			</tr>
