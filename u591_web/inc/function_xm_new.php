...
function WriteCard_money($tabType, $ServerID, $money, $PayID, $OrderID, $type=8, $i=0, $odds=0 ,$isgoods=0) {
	$i++;
	...
	if ($rows['count'] == 0) {
		$sql = "insert into $table(id,data,account_id,ref_id,time_stamp,used,type,server_id,back_odds,id_buygoods)";
		$sql = $sql . " values($nowId,'$money',$PayID,'$OrderID',$time_stamp,0,'$type','$ServerID',$odds,$isgoods)";
		if (mysqli_query ($conn, $sql ) == false) {
			write_log (ROOT_PATH."log", "card_err_", "sql=$sql, ".mysqli_error($conn)." ".date ("Y-m-d H:i:s")."\r\n");
			gameOrder($OrderID); //更新订单状态
			//执行失败再次请求
			if($i == 1){
				WriteCard_money($tabType, $ServerID, $money, $PayID, $OrderID, $type = 8, $i ,$odds,$isgoods);
			}
		} else {
...
function SAddData($data){
	$url = 'http://pokespgtj.u591776.com:8080/index.php/ApiPay/PaylogProcess';
