...
$get = serialize($_GET);
write_log(ROOT_PATH."log","leyou_callback_info_all_","post=$post, get=$get, ".date("Y-m-d H:i:s")."\r\n");
...
$mySign = md5($md5Str);
if($sign != $mySign){
    write_log(ROOT_PATH."log","leyou_callback_error_","sign=$sign, mySign=$mySign, md5Str=$md5Str, ".date("Y-m-d H:i:s")."\r\n");
    exit('failure');
}
$conn = SetConn(88);
$orderId = $orderid;
...
if(!$result){
    ...
    if(!$result_account['NAME']){
        write_log(ROOT_PATH."log","leyou_callback_error_", "account is not exist.  ".date("Y-m-d H:i:s")."\r\n");
        exit("failure");
    }else{
    ...
    $sql=$sql." VALUES (124, $accountId,'$PayName','$serverId','$payMoney','$orderId','$dwFenBaoID','$Add_Time','1','$gameId','$clienttype', '1')";
    
    if (mysqli_query($conn,$sql) == False){
        write_log(ROOT_PATH."log","leyou_callback_error_","sql=$sql, ".date("Y-m-d H:i:s")."\r\n");
        exit('failure');
    }
