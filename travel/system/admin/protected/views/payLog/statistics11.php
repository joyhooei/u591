	       	...
	       	<?php
				...
				if($type == 0) {
					foreach ($info as $k => $v) {
						...
						<tr>
							<td><?=$i ?></td>
							<td><?=$type == 1 ? date('Y-m-d', strtotime($startTime)) . '~' . date('Y-m-d', strtotime($endTime)) : $k; ?></td>
							<td><?=implode('；', $v['payTotal']);?></td>

							<td><?=$type == 1 ? (isset($channel[$v->CPID]) ? $channel[$v->CPID] : $v->CPID) : '全部'; ?></td>
							<td><?=implode('；', $v['payIdTotal']); ?></td>
						</tr>
						<?php
						$i++;
					}
